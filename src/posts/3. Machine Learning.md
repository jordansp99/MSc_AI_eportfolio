...
dtype: float64
```
Horsepower is an object but should be an int.
```python
...
This varible has mixed dtypes as '?' has been used for null.
```python
#dropping ?
...
```
Replacing the numeric values with categorical as it's unclear what the numeric values to refer to:
